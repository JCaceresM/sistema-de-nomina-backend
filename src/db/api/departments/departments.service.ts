...
export class DepartmentsService {
  async find(conditions: SelectConditionType[] = [], queryParams: QueryParams) {
    ...
    left join lateral 
    ( select validEmp.*,  COALESCE(json_agg(payroll_news) FILTER (WHERE payroll_news.amount  IS NOT NULL), '[]') AS payroll_news from (SELECT id, document_id, first_name, last_name, gender, marital_status, status, born_date, company_id, salary, updated_at, created_at, user_update, user_insert,
    department_id
    FROM employee e where d.id = e.department_id) as validEmp
    left join lateral 
(SELECT * FROM public.payroll_news AS pn 
  INNER JOIN payroll_news_relation r
  ON r.employee_id = validEmp.id and pn.id = r.payroll_news_id 

) as payroll_news on true
 GROUP BY validEmp.id, validEmp.document_id,validemp.first_name, validemp.last_name,validemp.gender,validemp.marital_status,validemp.status, validEmp.born_date, validEmp.company_id, validEmp.salary, validEmp.updated_at, validEmp.created_at, validEmp.user_update, validEmp.user_insert,
    validEmp.department_id
    )
  ...
  async findNotInPayroll(
    ...
    const statement2 = `
        ...
        and d.id in (select pd."departmentId"  from payroll_deparments pd where 1=1 ${await searchConditionQuery(
          conditions,
          'payroll_deparments',
          'pd',
        )})
